...
func (ml *MultiLocalTrans) Ping(v *Vnode) (bool, error) {
	if local, ok := ml.hosts[v.HostIdf]; ok {
		return local.Ping(v)
	}
	return ml.remote.Ping(v)
}

func (ml *MultiLocalTrans) SendMessage(v *Vnode, v2 *Vnode, message string, option int) (bool, error) {
	if local, ok := ml.hosts[v.HostIdf]; ok {
		return local.Ping(v)
	}
	return ml.remote.Ping(v)
...
func (ml *MultiLocalTrans) Notify(target, self *Vnode) ([]*Vnode, error) {
	if local, ok := ml.hosts[target.HostIdf]; ok {
...
func (ml *MultiLocalTrans) Register(v *Vnode, o VnodeRPC) {
	local, ok := ml.hosts[v.HostIdf]
	if !ok {
		local = InitLocalTransport(nil).(*LocalTransport)
		ml.hosts[v.HostIdf] = local
	}
	local.Register(v, o)
}
...
func (ml *MultiLocalTrans) SendHandShakeMessage(targetVn *Vnode, sourceVn *Vnode, key string, step int) ([]byte, error) {
	return nil, nil
}

func (ml *MultiLocalTrans) TransferData(targetVn *Vnode, dataID string, data []byte, option int) ([]byte, string, error) {
	return nil, "none", nil
}

func TestDefaultConfig(t *testing.T) {
	conf := DefaultConfig("test", GetOutboundIP()+":"+"5678")
