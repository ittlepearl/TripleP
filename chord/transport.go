...
func (lt *LocalTransport) SendMessage(vn *Vnode, srcVnode *Vnode, message string, option int) (bool, error) {
	// Look for it locally
	_, ok := lt.get(vn)

	// TODO IF THE DEST NODE IS LOCAL, IGNORE IT?
	// If it exists locally, handle it
	if ok {
		return true, nil
	}
	...
	return lt.remote.SendMessage(vn, srcVnode, message, option)
}

func (lt *LocalTransport) SendHandShakeMessage(targetVn *Vnode, sourceVn *Vnode, key string, step int) ([]byte, error) {
	// Look for it locally
	_, ok := lt.get(targetVn)

	// TODO IF THE DEST NODE IS LOCAL, IGNORE IT?
	// If it exists locally, handle it
	if ok {
		return nil, fmt.Errorf("ERROR: target is a local node")
	}

	// Pass onto remote
	// TODO HANDLE OFFLINE SCENARIO
	return lt.remote.SendHandShakeMessage(targetVn, sourceVn, sourceVn.HostIdf, step)
}

func (lt *LocalTransport) TransferData(targetVn *Vnode, dataID string, data []byte, option int) ([]byte, string, error) {
	// Look for it locally
	_, ok := lt.get(targetVn)

	// If it exists locally, handle it
	if ok {
		fmt.Println("store locally!")
		return nil, "local", nil
	}

	// Pass onto remote
	// TODO HANDLE OFFLINE SCENARIO
	return lt.remote.TransferData(targetVn, dataID, data, option)
}
...
func (*BlackholeTransport) SendMessage(vn *Vnode, srcVnode *Vnode, message string, option int) (bool, error) {
	return false, nil
}

func (*BlackholeTransport) SendHandShakeMessage(targetVn *Vnode, sourceVn *Vnode, key string, step int) ([]byte, error) {
	return nil, nil
}

func (*BlackholeTransport) TransferData(targetVn *Vnode, dataID string, data []byte, option int) ([]byte, string, error) {
	return nil, "none", nil
}
