...
func (vn *localVnode) init(idx int) {
	...
	vn.HostIdf = vn.ring.config.Hostname
	vn.Host = vn.ring.config.HostAddr
...
func (vn *localVnode) genId(idx uint16) {
	...
	//fmt.Printf("hash hostname %v is %v\n", conf.Hostname, hash.Sum(nil))
...
func (vn *localVnode) GetLocalRing() *Ring {
	return vn.ring
}
